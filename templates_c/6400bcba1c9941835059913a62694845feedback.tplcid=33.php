...
<html>
	...
	<?php include 'header.inc.php';?>
		<div id="feedback">
			...
			<p class="info"><?php echo $this->vars['info'];?>　<a href="details.php?id=<?php echo $this->vars['id'];?>" target="_blank">[详情]</a></p>
			
			<?php if($this->vars['HotThreeComment']){?>
			<?php foreach($this->vars['HotThreeComment'] as $key=>$value){?>
				<dl>
					<dt><img src="image/1.jpg"/></dt>
					<dd><em><?php echo $value->date ?>发表</em><span style="color: #06f;"><?php echo $value->user ?></span><img src="image/hot.gif" alt="热"/></dd>
					<dd class="info">（<?php echo $value->manner ?>） <?php echo $value->content ?></dd>
					<dd class="button">
					<a href="feedback.php?cid=<?php echo $value->cid ?>&id=<?php echo $value->id ?>&type=sustain" target="_blank">[<?php echo $value->sustain ?>]支持</a> 
			...
			<h4>最新评论</h4>
			<?php if($this->vars['AllComment']){?>
			<?php foreach($this->vars['AllComment'] as $key=>$value){?>
				<dl>
					<dt><img src="image/1.jpg"/></dt>
		...
		<div id="sidebar">
			<h2>热评文档总排行</h2>
			<?php if($this->vars['TwentyComment']){?>
			<?php foreach($this->vars['TwentyComment'] as $key=>$value){?>
			<ul>
				<li><em>08-13</em><a href="details.php?id=<?php echo $value->id ?>" target="_blank"><?php echo $value->title ?></a></li>
			</ul>
			<?php } ?>
			<?php } ?>
		</div>
		...
		<div class="d5">
			<form action="feedback.php?cid=<?php echo $this->vars['cid'];?>" method="post" name="comment" target="_blank">
